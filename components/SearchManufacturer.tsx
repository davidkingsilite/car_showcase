...
const SearchManufacturer = ({manufacturer, setManufacturer}: SearchManufacturerProps) => {
  ...
  return (
    <div className='search-manufacturer'>
       <Combobox  onClose={()=>setQuery('')} value={manufacturer} onChange={ setManufacturer}>
                  ...
                  Manufacturer not found.
                        ...
                        filteredManufacturers.map((item) => (
                            <ComboboxOption 
                               key={item}
                               value={item}
                               className={({focus})=>`relative search-muanufacturer__option ${focus ? 'bg-primary-blue text-white' :'text-gray-900'}`}
                               >
                                 {({ selected, focus }) => (
                      <>
                        <span
